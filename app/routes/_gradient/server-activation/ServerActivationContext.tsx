...
export const ServerActivationContext =
	createContext<ServerActivationContextType | null>(null);
...
export function ServerActivationProvider({
	...
	return (
		<ServerActivationContext.Provider
			value={{ serverSettings, setServerSettings }}
		>
			{children}
		</ServerActivationContext.Provider>
	);
...
export function useServerActivationContext() {
	...
	if (context === null) {
		throw new Error(
			"useServerActivationContext must be used within an ServerActivationProvider"
		);
	}
	return context;
}
